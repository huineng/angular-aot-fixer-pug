...
var fixer = function(options) {
  glob(options.srcPath + "/**/*.pug", {}, function(err, files) {
    files.forEach(function(fileName) {
      ...
      var fileContent = "export const template = `" + pugContent + "`;";
      fs.writeFileSync(fileName + ".ts", fileContent);
    });
  });

  glob(options.srcPath + "/**/*.ts.css", {}, function(err, files) {
    files.forEach(function(fileName) {
      var cssContent = fs.readFileSync(fileName, {
        encoding: 'utf8'
      });
      var fileContent = "export const styles = `" + cssContent.replace(/(\r\n|\n|\r)/gm, '').trim() +
        "`;";
      fs.writeFileSync(fileName.slice(0, -7) + "-css.ts", fileContent, 'utf8');
    });
  });
};
